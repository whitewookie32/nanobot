...
class LiteLLMProvider(LLMProvider):
    ...
    def __init__(
        ...
        self.use_litellm_proxy = bool(_get_setting("use_litellm_proxy", False))

        # Detect OpenRouter by api_key prefix or explicit api_base
        self.is_openrouter = (
            (api_key and api_key.startswith("sk-or-")) or
            (api_base and "openrouter" in api_base_lower)
        )

        # Detect Together AI by api_base or model prefix
        self.is_together = (
            (api_base and "together" in api_base_lower) or
            self.default_model.startswith("together_ai/")
        )

        # Detect AiHubMix by api_base
        self.is_aihubmix = bool(api_base and "aihubmix" in api_base_lower)

        # LiteLLM proxy should use OpenAI-compatible auth without hosted_vllm model rewrites.
        self.is_litellm_proxy = bool(api_base) and (
            self.use_litellm_proxy or "litellm" in api_base_lower
        )

        # Track if using raw custom OpenAI-compatible endpoint (vLLM, etc.).
        self.is_vllm = (
            bool(api_base)
            and not self.is_openrouter
            and not self.is_together
            and not self.is_aihubmix
            and not self.is_litellm_proxy
        )
        ...
        if api_key:
            ...
            elif self.is_vllm or self.is_litellm_proxy:
                # OpenAI-compatible endpoints (vLLM / LiteLLM Proxy)
                os.environ["OPENAI_API_KEY"] = api_key
            ...
            elif "zhipu" in self.default_model or "glm" in self.default_model or "zai" in self.default_model:
                os.environ.setdefault("ZHIPUAI_API_KEY", api_key)
            elif "dashscope" in self.default_model or "qwen" in self.default_model.lower():
                os.environ.setdefault("DASHSCOPE_API_KEY", api_key)
            elif "moonshot" in self.default_model or "kimi" in self.default_model.lower():
    ...
    async def chat(
        ...
        if not (self.is_vllm or self.is_litellm_proxy or self.is_openrouter or self.is_together):
