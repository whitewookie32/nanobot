...
"""
    ...
    def _extract_login_info(lines: list[str]) -> dict[str, str]:
        auth_url = ""
        callback_url = ""
        code = ""
    ...
    def _codex_login_start_subprocess(command: list[str], flow: str) -> dict:
    ...
    def _codex_login_start_device_auth() -> dict:
        return _codex_login_start_subprocess(
    ...
    def _codex_login_start() -> dict:
        ...
        fallback_to_device = os.getenv("NANOBOT_CODEX_DEVICE_FALLBACK", "0").lower() not in {
            "0",
            "false",
            "no",
            "off",
        ...
        if use_localhost:
            result = _codex_login_start_subprocess(["codex", "login"], "localhost_redirect")
            if result.get("ok") or not fallback_to_device:
                return result
    ...
    def _codex_submit_callback(callback_url: str) -> dict:
        callback_url = (callback_url or "").strip()
        if not callback_url:
            return {"ok": False, "error": "callbackUrl is required"}

        parsed = urlparse(callback_url)
        host = (parsed.hostname or "").lower()
        if parsed.scheme not in {"http", "https"}:
            return {"ok": False, "error": "callbackUrl must start with http:// or https://"}
        if host not in {"localhost", "127.0.0.1", "::1"}:
            return {"ok": False, "error": "callbackUrl host must be localhost/127.0.0.1/::1"}

        try:
            req = urllib_request.Request(callback_url, method="GET")
            ...
            with codex_lock:
                codex_state["lines"].append(
                    f"Callback forwarded to {host_port}{parsed.path or '/'} (HTTP {status_code})"
                )
                codex_state["lines"] = codex_state["lines"][-200:]
            return {"ok": ok, "statusCode": status_code, "output": snippet}
        except Exception as exc:
            with codex_lock:
                codex_state["lines"].append(f"Callback forwarding failed: {exc}")
                codex_state["lines"] = codex_state["lines"][-200:]
            return {"ok": False, "error": str(exc)}
    ...
    def _codex_snapshot() -> dict:
        ...
        login["device"] = _extract_login_info(lines)
        if not login["flow"]:
            login["flow"] = "device_code" if login["device"].get("code") else "localhost_redirect"
        ...
        class WebHandler(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:  # noqa: N802 - required by BaseHTTPRequestHandler
                ...
                if path == "/api/codex/logout":
                    ...
                    self._send_json(json.dumps(result), status)
                    return
                if path == "/api/codex/callback":
                    try:
                        body = self._read_body()
                        payload = json.loads(body or "{}")
                        callback_url = str(payload.get("callbackUrl", ""))
                        result = _codex_submit_callback(callback_url)
                        status = 200 if result.get("ok") else 400
                        self._send_json(json.dumps(result), status)
                    except Exception as exc:
    ...
    # Register Omi bridge routes
    from nanobot.bridge.omi_handler import register_routes
    register_routes(gateway)
    console.print("[green]✓[/green] Omi bridge: /tools/ask")
    ...
    # Initialize Omi handler with agent
    from nanobot.bridge import omi_handler
    omi_handler.set_agent_loop(agent)
