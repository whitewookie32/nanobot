...
"""
    ...
    def _extract_device_info(lines: list[str]) -> dict[str, str]:
        """Extract device code info from process output (legacy device flow)."""
        url = ""
        code = ""
    ...
    # NEW: Localhost redirect OAuth flow for Codex
    codex_oauth_server = None
    
    def _codex_login_start() -> dict:
        """Start Codex OAuth using localhost redirect flow."""
        from nanobot.providers.codex_oauth import CodexOAuthServer
        
        nonlocal codex_oauth_server
        
        # Check if using new OAuth flow or fallback to CLI device auth
        use_localhost_oauth = os.getenv("NANOBOT_CODEX_LOCALHOST_OAUTH", "1") == "1"
        
        if not use_localhost_oauth:
            # Fallback to old device code flow
            return _codex_login_start_device_auth()
        
        # Use new localhost redirect flow
        try:
            codex_oauth_server = CodexOAuthServer()
            flow_result = codex_oauth_server.start_flow()
            ...
            with codex_lock:
                codex_state.update({
                    ...
                    "lines": [
                        ...
                        f"Visit: {flow_result['url']}",
                        f"Or check your browser (should open automatically)",
                    ],
                    "error": None,
                })
            ...
            def _wait_for_oauth():
                try:
                    result = codex_oauth_server.wait_for_completion(timeout=300)
                    with codex_lock:
                        codex_state["running"] = False
                        codex_state["finished_at"] = time.time()
                        if result.get("ok"):
                            codex_state["exit_code"] = 0
                            codex_state["lines"].append("Authentication successful!")
                        else:
                            codex_state["exit_code"] = 1
                            codex_state["error"] = result.get("error", "Unknown error")
                            codex_state["lines"].append(f"Error: {result.get('error')}")
                except Exception as exc:
                    with codex_lock:
                        codex_state["running"] = False
                        codex_state["exit_code"] = 1
                        codex_state["error"] = str(exc)
                        codex_state["lines"].append(f"Exception: {exc}")
            
            Thread(target=_wait_for_oauth, daemon=True).start()
            
            return {
                "ok": True, 
                "flow": "localhost_redirect",
                "url": flow_result["url"],
                "port": flow_result["port"],
            }
            
        except Exception as exc:
            # Fallback to old flow on error
            console.print(f"[yellow]localhost OAuth failed ({exc}), falling back to device code flow...[/yellow]")
            return _codex_login_start_device_auth()
    
    def _codex_login_start_device_auth() -> dict:
        """Original device code flow (fallback)."""
        ...
        return {"ok": True, "flow": "device_code"}

    def _codex_logout() -> dict:
        """Logout from Codex - clears both CLI and OAuth tokens."""
        # Try new OAuth logout first if available
        try:
            from nanobot.providers.codex_oauth import CodexOAuthServer
            oauth = CodexOAuthServer()
            oauth.logout()
            oauth_logged_out = True
        except:
            oauth_logged_out = False
        
        # Try CLI logout as well
        cli_logged_out = False
        if _codex_installed():
            env, _ = _codex_env()
            try:
                result = subprocess.run(
                ...
                cli_logged_out = result.returncode == 0
            except:
                pass
        
        if oauth_logged_out or cli_logged_out:
            return {"ok": True, "output": "Logged out successfully"}
        
        return {"ok": False, "error": "No active session found to log out"}

    def _codex_snapshot() -> dict:
        """Get current Codex auth status and any ongoing login state."""
        ...
        # Check if this is localhost OAuth (has full URL in first line) or device flow
        if lines and "http://localhost:" in lines[0]:
            login["flow"] = "localhost_redirect"
        else:
            login["flow"] = "device_code"
            login["device"] = _extract_device_info(lines)
        
        # Check if new OAuth method token exists
        try:
            from nanobot.providers.codex_oauth import CodexOAuthServer
            oauth = CodexOAuthServer()
            oauth_status = oauth.get_status()
            if oauth_status.get("authenticated"):
                status["oauthToken"] = oauth_status
        except:
            pass
...
codex_app = typer.Typer(help="Manage Codex CLI authentication")
app.add_typer(codex_app, name="codex")


@codex_app.command("login")
def codex_login(
    device_code: bool = typer.Option(False, "--device-code", "-d", help="Use device code flow instead of localhost redirect"),
    port: int = typer.Option(9654, "--port", "-p", help="Port for localhost callback (default: 9654)"),
):
    """Authenticate with Codex CLI using OAuth."""
    from nanobot.providers.codex_oauth import CodexOAuthServer
    
    console.print(f"{__logo__} Codex Authentication\n")
    
    if device_code:
        console.print("[yellow]Using device code flow (fallback mode)...[/yellow]")
        # This would need the subprocess fallback implemented
        console.print("[red]Device code flow not yet implemented in this command.[/red]")
        console.print("Use: codex login --device-auth")
        raise typer.Exit(1)
    
    try:
        oauth = CodexOAuthServer(port=port)
        
        # Check existing status
        status = oauth.get_status()
        if status.get("authenticated"):
            console.print("[green]✓[/green] Already authenticated with Codex")
            console.print(f"   Token stored at: {status.get('token_path')}")
            if not typer.confirm("Re-authenticate?"):
                raise typer.Exit()
        
        # Start OAuth flow
        flow = oauth.start_flow()
        
        console.print("Starting OAuth authentication...")
        console.print(f"[cyan]→[/cyan] Local callback server: http://localhost:{flow['port']}/callback")
        console.print(f"[cyan]→[/cyan] Opening browser...")
        console.print(f"\n[dim]If browser doesn't open, visit:[/dim]")
        console.print(f"[blue]{flow['url']}[/blue]\n")
        
        console.print("Waiting for authentication...")
        
        result = oauth.wait_for_completion(timeout=300)
        
        if result.get("ok"):
            console.print("\n[green]✓[/green] Authentication successful!")
            console.print(f"   Token saved to: ~/.codex/auth.json")
            console.print(f"\nYou can now use codex commands.")
        else:
            console.print(f"\n[red]✗[/red] Authentication failed: {result.get('error', 'Unknown error')}")
            raise typer.Exit(1)
...
def codex_logout():
    ...
    if oauth.logout():
        console.print("[green]✓[/green] Logged out from Codex")
    else:
        console.print("[yellow]No active Codex session found[/yellow]")
...
def codex_status():
    ...
    console.print(f"{__logo__} Codex Status\n")
    
    if status.get("authenticated"):
        console.print("[green]✓[/green] Authenticated")
        if status.get("token_path"):
            console.print(f"   Token: {status['token_path']}")
    else:
        console.print("[dim]Not authenticated[/dim]")
        console.print("\nRun [cyan]nanobot codex login[/cyan] to authenticate")
